...
VectorXd loadMtxVector(const std::string& filename) {
    ...
    std::string line;
    // Ignore the header lines (comments starting with '%')
    ...
    /*if (cols != 1) {
		cerr << cols << " " << rows << endl;
        throw std::runtime_error("This is not a vector.");
    }*/

    VectorXd vec = VectorXd::Zero(rows);  // Initialize with zeros

    // Read the indexed vector entries
    int index;
    double value;
    for (int i = 0; i < rows; ++i) {
        file >> index >> value;
        vec(index - 1) = value;  // Convert 1-based index to 0-based for Eigen
    }
...
int main() {
	...
	saveAsPng(SHARPENED, reshapeToMatrix(vSharpened, M, N));		// TODO: is it correct???
	...
	system(".//lis-2.1.6//test//test1 A2.mtx w.mtx x.mtx hist.txt -i gs -tol 1.0e-9");

	xLIS = loadMtxVector("x.mtx");

	checkValues(xLIS);

	saveAsPng(XLIS, reshapeToMatrix(xLIS, M, N));
